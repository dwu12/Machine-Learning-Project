...
def generate(
    ...
    input_image=None,
    strength=0.8,       # how much attention we want to pay to the original starting image
    ...
    with torch.no_grad():
        ...
        if sampler_name == "ddpm":
            ...
            raise ValueError("Unknown sampler value {}. ".format(sampler_name))
        ...
        if input_image:
            ...
            input_image_tensor = np.array(input_image_tensor) # (Height, Width, Channel)
            input_image_tensor = torch.tensor(input_image_tensor, dtype=torch.float32) # (Height, Width, Channel)
            input_image_tensor = rescale(input_image_tensor, (0, 255), (-1, 1)) # (Height, Width, Channel)
            ...
            ## The more strength, the more noise will be added 
